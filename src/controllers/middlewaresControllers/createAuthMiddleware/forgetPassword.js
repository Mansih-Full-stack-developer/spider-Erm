...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const Spider_app_email = settings['Spider_app_email'];
  const Spider_base_url = settings['Spider_base_url'];

  const url = checkAndCorrectURL(Spider_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | Spider',
    Spider_app_email,
